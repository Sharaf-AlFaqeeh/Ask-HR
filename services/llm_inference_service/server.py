# llm_inference_service/server.py
import os
import sys
import time
import uuid
...
def startup_event():
    ...
    # List available models (both directories and .gguf files)
    if os.path.isdir(models_dir):
        available = [d for d in os.listdir(models_dir) if os.path.isdir(os.path.join(models_dir, d)) or d.endswith(".gguf")]
    ...
    for model_name in GGUF_MODEL_PRIORITY:
        model_path = os.path.join(models_dir, model_name)
        if not os.path.exists(model_path):
            logger.debug(f"GGUF Model {model_name} not found at {model_path}, skipping...")
            continue
            
        logger.info(f"🔄 Loading prioritized GGUF model: {model_name}")
        ...
        try:
            ...
            logger.info(f"   Engine: llama-cpp-python")
            ...
            logger.error(f"❌ Failed to load prioritized GGUF model {model_name}: {e}", exc_info=True)
...
def _handle_transformers_inference(request: ChatCompletionRequest, request_id: str, user_query: str) -> ChatCompletionResponse:
    ...
    generated_ids = model.generate(
        **inputs,
        ...
        temperature=0.01,         # اجعلها قريبة من الصفر للصرامة
        do_sample=False,          # استخدم التوليد الحتمي (Greedy) بدلاً من العشوائي
        repetition_penalty=1.5,   # ارفع العقوبة إلى 1.5 لمنع تكرار الجمل
        no_repeat_ngram_size=2    # هذا السطر سيمنع النموذج من تكرار نفس الكلمات مرتين
    )
